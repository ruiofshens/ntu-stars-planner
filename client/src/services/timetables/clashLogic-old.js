...
export const checkTimeClash = (lesson, timetable) => {
    ...
    for (let currLesson of module.index.lessons) {
      ...
      if (DaysEnum[lesson.day] === DaysEnum[currLesson.day]) {
        if (lesson.startTime < currLesson.endTime && lesson.endTime > currLesson.startTime) { 
          if (checkTeachingWeeksClash(lesson.teachingWeeks, currLesson.teachingWeeks)) {
            return true;
          }
        }
      }
...
export const checkExamClash = (examDocs) => {
...
/* Helper Functions */
const checkTeachingWeeksClash = (teachingWeeksA, teachingWeeksB) => {
  /**
  ...
  if (teachingWeeksA.includes("Teaching Wk") && teachingWeeksB.includes("Teaching Wk")) { // both have specific teaching weeks - so must check clash
    ...
    let arrayWeeksA = getIndivWeeks(teachingWeeksA);
    let arrayWeeksB = getIndivWeeks(teachingWeeksB);
...
const getIndivWeeks = (teachingWeeksStr) => {
  let weeksArray = [];
