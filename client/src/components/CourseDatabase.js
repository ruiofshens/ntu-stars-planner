...
function CourseDatabase() {
  ...
  const popover = (
    <Popover id="popover-basic">
      <Popover.Header as="h3" id="popover-basic-header">Legend</Popover.Header>
      <Popover.Body style={{whiteSpace:"pre-line"}} id="popover-basic-body">
        {"* : Available as UEs\n~ : Available as BDEs\n^ : Self-paced Course\n# : Available as GERPEs"}
      </Popover.Body>
    </Popover>
  );
    ...
    <Card className={`retrievedCourses ${customOptions.displaySetting}`} style={{height: "30rem"}} border={customOptions.displaySetting === "lightMode" ? "" : "secondary"}>
      <Card.Header className="database-title font-weight-bold">
        <Row className="align-items-center">
          <Col sm={7} xs={9} id="courses-list-title"> 
            Courses Available 
      ...
      {courses.length === 0 ? 
        <Container className="d-flex flex-column justify-content-center align-items-center h-100" id="loading-courses">
          <Spinner animation="grow" role="status" variant="secondary">
            <span className="sr-only" id="spinner-loading-courses">Loading...</span>
          </Spinner> 
          <Row className="text-center pt-1" id="text-loading-courses">
            Almost there...
          </Row>
        </Container> :

        <ListGroup variant="flush" className="overflow-auto">
          {dynamicSearch().map((course, i) => {
            return (
              <ListGroup.Item 
              key ={course.courseCode} 
              variant={customOptions.displaySetting === "lightMode" ? "light" : "dark"}
              id={`option-course-${i}-${course.courseCode}`}>
                <Button 
                  variant="outline-primary" 
                  className="mx-1"
                  size="sm"
                  onClick= {() => addSelection(course.courseCode)}
                  id={`btn-add-course-${i}-${course.courseCode}`}
                  >+
                </Button>
